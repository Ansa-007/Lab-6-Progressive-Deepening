...
Progressive deepening is a search technique in AI that combines the ideas of depth-first search (DFS) and breadth-first search (BFS). It works by doing a DFS up to a certain depth limit, then increasing that limit step by step until it finds the goal. Like DFS, it uses little memory, and like BFS, it always finds the shortest (best) solution. Even though it repeats some searches, it’s an efficient way to explore when the goal depth is unknown.
...
- *A heartfelt gratitude for my instructor **Sir! Muhammad Imran Afzal** for his continous guidance and a firm commitment to provide the valuable time to polish me for industry-ready experience.*
